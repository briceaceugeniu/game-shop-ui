...
const Register = () => {
  ...
  const [passwordConfirmSuccess, setPasswordConfirmSuccess] =
    useState<InputProps>(defaultInput);
  ...
  const handleRegisterClicked = () => {
    ...
    axios
      .post(API_BASE_URL + "/customers/register", {
        email: emailValue,
        name: nameSuccess.value,
        password: passwordSuccess.value,
      })
      .then(function (response) {
        ...
        if (response.status === 201) {
          setAlert({ visible: true, type: "success", text: response.data });
